...
public class ThreadOddEvenCompletableFeature {
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        CompletableFuture.runAsync(() -> {
            ThreadOddEvenCompletableFeature.printResult(even);
        });
        CompletableFuture.runAsync(() -> {
            ThreadOddEvenCompletableFeature.printResult(odd);
        });
    ...
    private static void printResult(IntPredicate cond) {
        IntStream.rangeClosed(1, 10).filter(cond)
                .forEach(ThreadOddEvenCompletableFeature::execute);
    }
